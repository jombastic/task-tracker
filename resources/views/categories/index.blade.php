<x-layouts.dashboard-layout title="Categories">
    ...
    <div class="bg-white rounded shadow divide-y">
        @forelse ($categories as $category)
            <div class="flex flex-col gap-4 p-4 sm:flex-row sm:justify-between sm:items-center">
                <div>
                    <p class="font-medium break-words">{{ $category->name }}</p>
                ...
                <div class="flex flex-row gap-3 justify-start sm:justify-end">
                    @can('update', $category)
                        <a href="{{ route('categories.edit', $category) }}" class="text-sm text-blue-600 hover:underline">
                            Edit
                        </a>
                    @endcan
                    ...
                    @can('delete', $category)
                        <form method="POST" action="{{ route('categories.destroy', $category) }}"
                            onsubmit="return confirm('Delete this category?')" class="flex">
                            @csrf
                            @method('DELETE')
